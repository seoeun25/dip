...
public class SimpleSchemaRegistryClientTest {
    ...
    public void test() {
        ...
        SchemaRegistryClient client = new SimpleSchemaRegistryClient(BASE_URL);

        String topic = "employee";

        try {
            SchemaInfo schemaInfo = client.getLatestSchemaByTopic(topic);
            ...
            try {
                SchemaInfo schemaInfo4 = client.getLatestSchemaByTopic("haha-test");
                Assert.fail();
            } catch (SchemaClientException e) {
                Assert.assertTrue(e.getMessage().contains("Schema Not Found"));
            }

            List<SchemaInfo> list = client.getLatestSchemaAll();
